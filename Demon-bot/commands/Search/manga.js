...
module.exports = {
  ...
  execute: async (client, message, args) => {
    ...
    .setColor('YELLOW')
    .setDescription(`Searching for manga titled **${query}** on  [MyAnimeList](https://myanimelist.net 'Homepage').`)
    .setThumbnail('https://i.imgur.com/u6ROwvK.gif')
    .setFooter(`Manga Query with MAL | \©️${new Date().getFullYear()} Demon`);
    ...
    for (const res of data.results.slice(0,10)) {
      pages.add(
        ...
        .setFooter([
          ...
          `Manga Query with MAL | \©️${new Date().getFullYear()} Demon`
        ].join('\u2000\u2000•\u2000\u2000'))
        .addFields([
    ...
    msg = await msg.edit(pages.firstPage).catch(()=>null) || await message.channel.send({embeds:[pages.firstPage]});
