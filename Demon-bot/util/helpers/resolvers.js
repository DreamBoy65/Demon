...
const resolveMember = async ({ message, search, useMessageContent = false }) => {
...
const resolveRole = async ({ message, search }) => {
	const contentToCheck = search || message.content;
	if (!contentToCheck || typeof contentToCheck !== "string") return new Error("String is missing to resolve role")
	// Try by parsing the search
	if (contentToCheck.match(/^<@&([0-9]{18})>/)) {
